...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'POST') {
      const { nombre, email, tipo } = req.body; // ✅ FIX: sin JSON.parse
      // ⚠️ NOTA DE SEGURIDAD: En producción hashear la contraseña con bcrypt antes de insertar
      await sql`
        INSERT INTO usuario (nombre_usuario, email, password, tipo_usuario)
